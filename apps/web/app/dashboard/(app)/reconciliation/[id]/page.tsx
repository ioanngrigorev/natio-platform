...
export default async function ReconciliationBatchPage({ params, searchParams }: { params: { id: string }; searchParams: Record<string, string | undefined> }) {
  const api = serverApi();
  let b: Detail;
  try {
    b = await api.get<Detail>(`/dashboard/reconciliation/batches/${params.id}${qs({ status: searchParams.status })}`);
  } catch (err) {
  ...
  return (
    <>
      <PageHeader
        breadcrumb={
          <Link href="/dashboard/reconciliation" className="hover:underline">
            Reconciliation
          </Link>
        }
        title={
        ...
        <Card title="Batch">
          <DescriptionList
            ...
            items={[
              ...
              { label: "Provider account", value: b.provider_account_name ?? "—" },
              { label: "Provider account id", value: <Mono>{b.provider_account_id}</Mono> },
            ...
            <tbody>
              {b.items.map((it) => (
                <tr key={it.id}>
                  ...
                  <td>
                    {it.payment_id ? (
                      <Link href={`/dashboard/payments/${it.payment_id}`} className="text-brand-600 hover:underline">
                  ...
                  <td>{it.provider_reference ? <Mono>{it.provider_reference}</Mono> : <span className="text-ink-400">—</span>}</td>
                  ...
                  <td className="max-w-[280px] truncate text-ink-600" title={it.notes ?? undefined}>
                    {it.notes ?? "—"}
                  </td>
                </tr>
              ))}
